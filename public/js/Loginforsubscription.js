...
function login() {
    ...
    let userCompany = document.getElementById("company_field").value;
...
function updateToken(company) {
    //checking available tokens
    firebase.database().ref('/company/' + company).once('value').then(function(snapshot) {
        ...
        /*if (lastupdated != currentMonth) {
            if (tokens < 5) {
                ...
                let updates = {};
                updates['/company/' + company + '/tokens/'] = postData;
